...
Версия релиза задаётся в `web/VERSION` и footer HTML; тег GitHub — `v` плюс версия. `web/UPDATE_FORMAT` равен `1`. Перечень устанавливаемых файлов зафиксирован в `web/updater.py:FILES`; при изменении схемы/добавлении обязательных файлов повышайте формат, чтобы старый updater отказался от несовместимого обновления. Изменённые бинарники VPN всегда требуют ручной установки. Публикуйте архив и файл `.sha256` вместе; заголовок/тег пакета должен совпадать с VERSION.
